...
Table \@ref(tab:descriptive_stats) shows full data's summary statistics.^[You can also produce statistics for a sub set of variables; see Table \@ref(table:stats_short).] `stargazer()` is an excellent solution to export outputs.
...
```{r fig:wages-tenure, echo=FALSE,fig.cap="Log wages vs. tenure"}
plot(x=tenure,y=ln_wage)
```

An alternative could be to export the figure and include it as an image.

```{r fig:figure-2, fig.cap="Alternative solution",echo =FALSE,results='hide'}
...
```{r regressions, echo=TRUE, results='asis'}
